...
// No fallback prices â€” all data comes from the Google Sheet only.
...
function renderGrid(priceList) {
  ...
  // Group prices by Brand â€” order comes from the sheet
  const brandGroups = {};
...
function initGlobe() {
  ...
  function updateGlobe() {
    ...
    globeItems.forEach((item) => {
      ...
      const angle = ratio * -55;           // max Â±55Â° tilt
      const scale = 1 - absRatio * 0.22;   // 1.0 â†’ 0.78 at edges
      const opacity = 1 - absRatio * 0.65;   // 1.0 â†’ 0.35 at edges
...
function toggleCard(card) {
  ...
  document.querySelectorAll('.counter-box.expanded, .globe-item.expanded').forEach(other => {
    if (other !== card) {
      ...
      if (otherIcon) otherIcon.textContent = 'ï¼‹';
  ...
  callbackForm.addEventListener("submit", function (e) {
    ...
    fetch(SCRIPT_URL, {
      ...
      .then(() => {
        ...
        // â”€â”€ Notify owner via WhatsApp â”€â”€
